...
export default function JyotishContributePage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/contribute/jyotish', {
      method: 'POST',
  ...
  if (success) return (
    <div className="min-h-screen bg-amber-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl p-10 shadow-md text-center max-w-md">
        <div className="text-5xl mb-4">✅</div>
        <h2 className="text-2xl font-bold text-amber-900 mb-2">Contribution Submitted!</h2>
        <p className="text-amber-700 mb-6">Your Jyotish contribution is under review. Outstanding submissions may be preserved on the blockchain.</p>
        <button onClick={() => router.push('/')} className="bg-amber-700 text-white px-6 py-2 rounded-lg">Go Home</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-amber-50">
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="mb-8">
          <a href="/contribute" className="text-amber-700 hover:underline text-sm">← Back to Contribute</a>
          <h1 className="text-3xl font-bold text-amber-900 mt-2">Jyotish Acharya Contribution</h1>
          <p className="text-amber-700 mt-1">Share your Vedic knowledge of graha remedies, muhurtas, and go-daan guidance.</p>
        </div>

        <div className="bg-white rounded-2xl shadow-md p-8 space-y-6">
          <div className="border-b border-amber-100 pb-6">
            <h2 className="font-bold text-amber-900 mb-4">Your Credentials</h2>
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input name="expert_name" value={form.expert_name} onChange={handleChange}
                  className="w-full border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500"
              ...
              <div>
                ...
                <input name="qualification" value={form.qualification} onChange={handleChange}
                  className="w-full border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500"
                  placeholder="Jyotish Acharya, Shastra Visharad..." />
              </div>
              <div>
                <label className="block text-sm font-medium text-amber-800 mb-1">Institution / Gurukul</label>
                <input name="institution" value={form.institution} onChange={handleChange}
                  className="w-full border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500"
                  placeholder="Gurukul or institution name" />
              </div>
              <div>
                <label className="block text-sm font-medium text-amber-800 mb-1">Years of Practice</label>
          ...
          <div className="border-b border-amber-100 pb-6">
            <h2 className="font-bold text-amber-900 mb-4">Your Contribution</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-amber-800 mb-1">Title *</label>
                <input name="title" value={form.title} onChange={handleChange}
                  className="w-full border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500"
                  placeholder="e.g. Shani Graha Shanti through Go-seva: Muhurta and Vidhi" />
              </div>
              <div>
                ...
                <textarea name="content" value={form.content} onChange={handleChange} rows={10}
                  className="w-full border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-500"
                  placeholder="Share your Jyotish knowledge — graha remedies, auspicious timing, scripture references, personal insights from practice..." />
              </div>
            </div>
          </div>

          <div className="border-b border-amber-100 pb-6">
            <h2 className="font-bold text-amber-900 mb-4">Media Attachments</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-amber-800 mb-2">Images & PDFs</label>
                <label className="cursor-pointer border-2 border-dashed border-amber-300 rounded-lg px-6 py-3 text-amber-700 hover:border-amber-500 transition-colors inline-block">
              ...
              <div>
                ...
                {form.youtube_urls.map((url, i) => (
                  <input key={i} value={url} onChange={e => handleYoutube(i, e.target.value)}
                    className="w-full border border-amber-200 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:border-amber-500"
                    placeholder="https://youtube.com/watch?v=..." />
                ))}
                <button onClick={addYoutube} className="text-sm text-amber-700 hover:underline">+ Add another YouTube link</button>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleSubmit} disabled={submitting}
              className="bg-amber-700 text-white px-8 py-3 rounded-lg hover:bg-amber-800 disabled:opacity-50 font-medium">
