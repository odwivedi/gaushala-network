...
${content.substring(0, 2000)}`
      ...
      logger.error('CONTRIBUTE', 'contribute/jyotish/route.ts', 'AI check failed', { err: String(aiErr) });
    }
    ...
    const result = await pool.query(
      ...
      [user.id, 'jyotish', title, content, expert_name, qualification, institution || null, experience_years || null, youtube_urls || [], ai_score, ai_reasoning]
    );
    ...
    logger.info('CONTRIBUTE', 'contribute/jyotish/route.ts', 'Jyotish contribution submitted', { contributionId, userId: user.id, ai_score });

    return NextResponse.json({ success: true, id: contributionId, ai_score });
  } catch (err) {
    logger.error('CONTRIBUTE', 'contribute/jyotish/route.ts', 'POST failed', { err: String(err) });
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT ec.*, u.display_name as contributor_name, u.email as contributor_email,
       ...
       WHERE ec.contribution_type = 'jyotish'
       GROUP BY ec.id, u.display_name, u.email
       ORDER BY ec.created_at DESC`
    );
    return NextResponse.json(result.rows);
  } catch (err) {
    logger.error('CONTRIBUTE', 'contribute/jyotish/route.ts', 'GET failed', { err: String(err) });
