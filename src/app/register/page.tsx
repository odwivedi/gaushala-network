...
export default function RegisterPage() {
  ...
  async function handleRegister() {
    ...
    try {
      ...
      if (data.success) {
        logger.info('UI', 'register/page.tsx', 'Registration successful', { email });
        router.push('/login?registered=1');
      } else {
        setError(data.error);
      }
    } catch (err) {
      logger.error('UI', 'register/page.tsx', 'Registration failed', { err: String(err) });
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', background: '#F5F5F0', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: '#fff', borderRadius: 12, padding: '2rem', width: '100%', maxWidth: 420, boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
        <h1 style={{ color: '#3B6D11', fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.25rem' }}>Create account</h1>
        <p style={{ color: '#666', marginBottom: '1.5rem', fontSize: '0.95rem' }}>
          Already have an account? <Link href="/login" style={{ color: '#3B6D11', fontWeight: 600 }}>Sign in</Link>
        </p>
        ...
        <div style={{ display: 'grid', gap: '1rem', marginBottom: '1.25rem' }}>
          <div>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.3rem', color: '#333', fontSize: '0.9rem' }}>Display name</label>
          ...
          <div>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.3rem', color: '#333', fontSize: '0.9rem' }}>Password <span style={{ color: '#999', fontWeight: 400 }}>(min 8 characters)</span></label>
        ...
        <p style={{ color: '#999', fontSize: '0.8rem', marginTop: '1rem', textAlign: 'center' }}>
          By registering you agree to contribute under CC BY-SA licence.
        </p>
